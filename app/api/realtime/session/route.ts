import { NextResponse } from "next/server";
import { voiceModels } from "@/config/models";

export const runtime = "nodejs";

export async function POST(request: Request) {
  const apiKey = process.env.OPENAI_API_KEY?.trim();
  if (!apiKey) return NextResponse.json({ error: "Voice service is not configured." }, { status: 503 });

  const sdp = await request.text();
  if (!sdp || sdp.length > 100_000) {
    return NextResponse.json({ error: "Invalid WebRTC offer." }, { status: 400 });
  }

  const session = {
    type: "realtime",
    model: voiceModels.realtime,
    instructions: [
      "# Role and Objective",
      "You are a practical voice assistant. State the answer directly.",
      `Today is ${new Date().toISOString().slice(0, 10)}.`,
      "# Verbosity",
      "Direct answers: use 1–2 short sentences. Ask only one clarifying question at a time.",
      "Remove introductions, repetition, filler, generic praise, and sign-offs.",
      "# Reasoning and Preambles",
      "Answer simple questions immediately. Use a single short preamble only before a tool call that may take noticeable time.",
      "# Unclear Audio",
      "Only respond to audio you understand clearly. If audio is ambiguous, noisy, cut off, or uncertain, ask the user to repeat it briefly. Never guess missing words.",
      "# Tools",
      "Use web_search before answering current or recent facts, news, politics, public officeholders, schedules, prices, weather, or information likely to have changed.",
      "After web search, answer from the result and briefly name the source and date when useful.",
    ].join("\n\n"),
    reasoning: { effort: "minimal" },
    output_modalities: ["audio"],
    audio: {
      input: {
        transcription: { model: "gpt-4o-transcribe" },
        turn_detection: { type: "semantic_vad", eagerness: "medium", create_response: true, interrupt_response: true },
      },
      output: { voice: "marin" },
    },
    tools: [
      {
        type: "function",
        name: "web_search",
        description: "Search the live web for current, recent, or time-sensitive factual information.",
        parameters: {
          type: "object",
          properties: {
            query: { type: "string", description: "A focused search query containing the facts that must be verified." },
          },
          required: ["query"],
        },
      },
    ],
    tool_choice: "auto",
  };
  const formData = new FormData();
  formData.set("sdp", sdp);
  formData.set("session", JSON.stringify(session));

  try {
    const response = await fetch("https://api.openai.com/v1/realtime/calls", {
      method: "POST",
      headers: { Authorization: `Bearer ${apiKey}` },
      body: formData,
    });
    const answer = await response.text();

    if (!response.ok) {
      console.error("Realtime session failed", response.status, answer);
      return NextResponse.json({ error: "The realtime session could not be started." }, { status: 502 });
    }

    return new NextResponse(answer, { headers: { "Content-Type": "application/sdp" } });
  } catch (error) {
    console.error("Realtime session failed", error);
    return NextResponse.json({ error: "The realtime session could not be started." }, { status: 502 });
  }
}
