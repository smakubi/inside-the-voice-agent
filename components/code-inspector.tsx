  ...
  return (
    <aside className="fixed inset-x-3 bottom-3 z-40 max-h-[82vh] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/20 sm:inset-y-4 sm:right-4 sm:left-auto sm:w-[520px]" aria-label="Python code inspector">
      <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-5 py-4">
        <div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-blue-700">Python example</p><h2 className="mt-1 text-xl font-semibold text-slate-950">{snippet.title}</h2><p className="mt-1 font-mono text-xs text-slate-500">{snippet.path} · {snippet.technology}</p></div>
      ...
      <div className="max-h-[calc(82vh-105px)] overflow-y-auto p-5">
        ...
        <div className="mt-4 overflow-hidden rounded-xl bg-slate-950">
          ...
          <pre className="overflow-x-auto p-4 text-[13px] leading-6 text-slate-200"><code><PythonCode code={snippet.code} /></code></pre>
        </div>
      </div>
    </aside>
  );
